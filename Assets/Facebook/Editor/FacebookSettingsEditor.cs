        ...
        if (showAndroidUtils)
        {
            if (!FacebookAndroidUtil.IsSetupProperly())
            {
                ...
                switch (FacebookAndroidUtil.SetupError)
                {
					...
					msg = "You don't have the Android SDK setup!  Go to " + (Application.platform == UnityEngine.RuntimePlatform.OSXEditor ? "Unity" : "Edit") + "->Preferences... and set your Android SDK Location under External Tools";
                        break;
